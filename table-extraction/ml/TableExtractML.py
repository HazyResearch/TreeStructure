...
#        filtered_bboxes = []
#        for bb in lines_bboxes:
#            if (bb[5]-bb[3] > 0 and bb[6]-bb[4]>0):
#                filtered_bboxes += [bb]
...
#        for bb in alignments_bboxes:
#            if (bb[5]-bb[3] > 0 and bb[6]-bb[4]>0):
#                filtered_bboxes += [bb]
        filtered_bboxes = alignments_bboxes + lines_bboxes
        if len(filtered_bboxes) == 0:
            return [], []
        lines_features = get_lines_features(filtered_bboxes, elems)
        features = np.concatenate((np.array(alignment_features), np.array(lines_features)), axis=1)
        return filtered_bboxes, features
