...
def extract_text_candidates(boxes, page_bbox, avg_font_pts, width, char_width, page_num, ref_page_seen, boxes_figures, page_width, page_height):
    ...
    while(True):
        for i1, b1 in enumerate(boxes):
            for i2, b2 in enumerate(boxes):
                ...
                box1 = b1.bbox
                box2 = b2.bbox
                if((abs(box1[1]-box2[1])<0.11*avg_font_pts) or ((abs(box1[3]-box2[3])<0.11*avg_font_pts)) or (round((box1[1]+box1[3])/2)==round((box2[1]+box2[3])/2))):
                    min_i = min(i1, i2)
                    max_i = max(i1, i2)
                    rid1 = obj2rid[min_i]
                    rid2 = obj2rid[max_i]
    ...
    while(True):
        for i1, b1 in enumerate(boxes):
            for i2, b2 in enumerate(boxes):
                ...
                box2 = b2.bbox
                if(obj2rid[i1] == obj2rid[i2]):
                    if(((b1.bbox[0]<b2.bbox[0]) and ((b2.bbox[0]-b1.bbox[2]) <= 2*char_width)) or ((b2.bbox[0]<b1.bbox[0]) and ((b1.bbox[0]-b2.bbox[2]) <= 2*char_width))):
                        min_i = min(i1, i2)
                        max_i = max(i1, i2)
                        cid1 = obj2cid[min_i]
                        cid2 = obj2cid[max_i]
        ...
        for i1, b1 in enumerate(boxes):
            for i2, b2 in enumerate(boxes):
                ...
                if(abs((box2[3]-box2[1])-(box1[3]-box1[1]))>0.5*avg_font_pts):
                    continue
                if(box2[1]<box1[3] or (box2[1]-box1[1]<1.5*avg_font_pts) or (box2[3]-box1[3]<1.5*avg_font_pts)): #can probably do better if we find the average space between words
                    if(abs(box1[0]-box2[0])<3*char_width or abs(box1[2]-box2[2])<3*char_width or (((box1[0]+box1[2])/2)==((box2[0]+box2[2])/2))):
                        min_i = min(i1, i2)
                        max_i = max(i1, i2)
                        cid1 = obj2cid[min_i]
                        cid2 = obj2cid[max_i]
    ...
    for cid in range(len(cid2obj)):
        if(len(cid2obj[cid]) == 1):
            if(boxes[list(cid2obj[cid])[0]].get_text().lower() == "references"):
                references_bbox = [cid2span[cid]["min_x"], cid2span[cid]["min_y"], cid2span[cid]["max_x"], cid2span[cid]["max_y"]]
                for cid2 in range(len(cid2obj)):
                    if(round(cid2span[cid]["min_x"]) == round(cid2span[cid2]["min_x"]) and cid2span[cid]["max_y"] < cid2span[cid2]["min_y"]):
    ...
    if(ref_page_seen == False):
        for cid in range(len(cid2obj)):
            if(len(cid2obj[cid])>0 and cid not in empty_cids and cid not in references_cid):
                ...
                rid_miny = {}
                for rid in rid_list:
                    rid_miny[rid] = min([boxes[obj].bbox[1] if obj in cid2obj[cid] else 10000 for obj in rid2obj[rid]])
                sorted_rid_miny = sorted(rid_miny.items(), key=operator.itemgetter(1))
                last_rid = 0
                for i in range(len(sorted_rid_miny)-1):
                    row1 = sorted_rid_miny[i][0]
                    row2 = sorted_rid_miny[i+1][0]
                    row1_maxx = max([boxes[obj].bbox[2] if obj in cid2obj[cid] else -1 for obj in rid2obj[row1]])
                    row2_minx = min([boxes[obj].bbox[0] if obj in cid2obj[cid] else 10000 for obj in rid2obj[row2]])
                    if(row1_maxx<=cid_maxx and (row2_minx-char_width)>cid_minx):
                        #split cluster cid
                        new_cid_idx = empty_cids[empty_idx]
                        empty_idx += 1
                        for i_iter in range(last_rid, i+1):
                            obj_list = [obj for obj in rid2obj[sorted_rid_miny[i_iter][0]] if obj2cid[obj]==cid]
                            for obj in obj_list:
    ...
    if page_num == -1:
        ...
        for idx, node in enumerate(new_nodes):
            ...
            first_elem = None
            for elem in node.elems:
                if(round(elem.bbox[0]) == round(node.x0) and round(elem.bbox[1]) == round(node.y0)):
                    first_elem = elem
                    break
            if(first_elem!=None):
            ...
            if(references_bbox != [] or ref_seen_in_node):
                if(node.y0>references_bbox[3] and node.x0<=references_bbox[0] and node.x1>references_bbox[2]):
                    node.type = "List"
                    continue
        ...
        for idx2, node2 in enumerate(new_nodes):
            if(idx1 != idx2):
                if(node2.type == "List" and ref_indices[idx2]==False):
                    if((node2.x0<=x0 and node2.x1>=x0) or (x0<=node2.x0 and x1>=node2.x0)):
